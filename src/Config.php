...

{
    ...
    {
        // 加载初始化文件
        Config::load(WG_CONFIG_PATH . 'config' . WG_CONF_EXT);

        // 读取扩展配置文件
        if (is_dir(WG_BASE_PATH . 'extra')) {
            $dir   = WG_BASE_PATH . 'extra';
            $files = scandir($dir);

            foreach ($files as $file) {
                if ('.' . pathinfo($file, PATHINFO_EXTENSION) === WG_CONF_EXT) {
    ...
    public static function parse($config, $type = '', $name = '', $range = '')
    {
        ...
        $class = false !== strpos($type, '\\') ?
            $type :
            '\\WangGe\\Cache\\driver\\config\\' . ucwords($type);
    ...
    public static function get($name = null, $range = '')
    {
        ...
        // 二维数组设置和获取支持
        $name    = explode('.', $name, 2);
        $name[0] = trim(strtolower($name[0]));

        if (!isset(self::$config[$range][$name[0]])) {
            // 动态载入额外配置
            $file   = WG_BASE_PATH . 'extra' . DS . $name[0] . WG_CONF_EXT;
